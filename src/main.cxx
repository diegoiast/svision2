...
class PlatformX11
{
    std::vector<std::shared_ptr<PlatformWindow>> windows;
    bool exit_loop = false;

public:
    std::shared_ptr<PlatformWindow> open_window(int x, int y, int width, int height, const char *title);
...
auto PlatformX11::open_window(int x, int y, int width, int height, const char *title) -> std::shared_ptr<PlatformWindow>
{
    auto window = std::make_shared<PlatformWindowX11>();

    window->dpy = XOpenDisplay(NULL);
    int screen = DefaultScreen(window->dpy);
    window->w = XCreateSimpleWindow(window->dpy, RootWindow(window->dpy, screen), 0, 0, width,
                                    height, 0, BlackPixel(window->dpy, screen),
                                    WhitePixel(window->dpy, screen));
    window->gc = XCreateGC(window->dpy, window->w, 0, 0);
    XSelectInput(window->dpy, window->w,
                 ExposureMask | KeyPressMask | KeyReleaseMask | ButtonPressMask |
                     ButtonReleaseMask | PointerMotionMask);
    XStoreName(window->dpy, window->w, title);
    XMapWindow(window->dpy, window->w);
    XSync(window->dpy, window->w);
    window->img = XCreateImage(window->dpy, DefaultVisual(window->dpy, 0), 24, ZPixmap, 0,
...
auto PlatformX11::main_loop() -> void
{
    ...
    for (auto ff : windows)
    {
        ...
        // XFlush(f->dpy);
        auto f = static_cast<PlatformWindowX11 *>(ff.get());
        while (p = XPending(f->dpy) || ! this->exit_loop)
        {
            auto k = XNextEvent(f->dpy, &ev);
            std::cout << l << ": p = " << p << " k = " << k << std::endl ;
            l++;
